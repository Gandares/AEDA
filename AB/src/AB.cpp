...
void ArbolB::pornivel(nodo* node){
...
void ArbolB::insertar1(int opcion){
    ...
    if(opcion==2){
        long int numero;
        cout << "Introduzca DNI (8 cifras): ";
        cin >> numero;
        dni DNI(numero);
        insertar(raiz_,DNI);
    }
}
void ArbolB::insertar(nodo* &node, dni key){
    
    if(node==NULL){
        node = new nodo(key);
    }
    if (key.get_num() < node->get_dato())
       insertar(node->get_izq(),key);
    else if (key.get_num() > node->get_dato())
        insertar(node->get_dcho(),key);
}

void ArbolB::buscar(nodo* node, long int key) const{
    ...
    if (node==NULL)
       ...
       return buscar(node->get_dcho(), key);
    else if (node->get_dato() > key)
       return buscar(node->get_izq(), key);
